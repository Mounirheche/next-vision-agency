...
async function read(name, defaultValue) {
  if (supabase.enabled) {
    ...
    if (!data) {
      // A null default (e.g. auth.js probing "does an account exist yet?") is a
      // sentinel, not a real value — the kv_store.value column is NOT NULL, so
      // only persist an actual default, never write null itself.
      if (defaultValue !== null && defaultValue !== undefined) await write(name, defaultValue);
      return defaultValue;
    }
    return data.value;
  }
